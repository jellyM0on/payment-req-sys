...
interface UsersTableContainerPropsInterface{
    users: Users[] | null,
    // handleUsersChange: (user:Users) => void
}
...
const EditRoleContainer = ({role, id, handleUpdate}:EditRoleContainerInterface) => {
...
const EditManagerContainer = ({manager, id, role}: EditManagerContainerProps) => {
    const [isEditable, setIsEditable] = useState(false)
    const [selectedManagerOpt, setSelectedManagerOpt] = useState<SingleComboBoxOption>();
    const [selectedManager, setSelectedManager] = useState<Manager | null>(manager)
    ...
    const updateManager = async () => {
        try{
            ...
            credentials: 'include',
            body: JSON.stringify({role: role, manager_id: selectedManager ? selectedManager.id : null} )
          }); 
    ...
    const handleManagerSelect = (opt: SingleComboBoxOption<never> | undefined) => {
    ...
    return(
        ...
        <Text>{selectedManager ? selectedManager.name : "N/A"}</Text >
         ...
         :  
         <IconOnlyButton  onClick={handleManagerEdit} label="edit" appearance="tertiary" IconComponent={MdModeEdit}>
         </IconOnlyButton> 
         }
...
function UsersTableContainer({users}: UsersTableContainerPropsInterface){
    ...
    const [updatedUsers, setUpdatedUsers] = useState<Users[] | null>(users)
          ...
          for(let i = 0; i < users.length; i++){
            ...
            console.log(cUser.manager)
              rows.push( {cells: [
                ...
                { value: 
                <EditRoleContainer role={cUser.role} id={cUser.id} handleUpdate={handleUpdate}/>
                }, 
                { value: <Text>{cUser.email}</Text> },
                { value: <Text>{cUser.position}</Text> }, 
                { value: <Text>{cUser.department}</Text> }, 
                { value: 
                <EditManagerContainer id={cUser.id} manager={cUser.manager ? cUser.manager : null} role={cUser.role} /> 
                }, 
